...
namespace HeroesProfile_Backend
{
    internal class ParseStormReplay
    {
        ...
        public ParseStormReplay(long replayID, Uri replayURL, HotsApiJSON.ReplayData hotsapi_data, Dictionary<string, string> maps, Dictionary<string, string> maps_translations,
                                Dictionary<string, string> game_types, Dictionary<string, string> talents, Dictionary<string, string> seasons_game_versions, Dictionary<string, string> mmr_ids,
                                Dictionary<string, DateTime[]> seasons, Dictionary<string, string> heroes, Dictionary<string, string> heroes_translations, Dictionary<string, string> maps_short,
                                Dictionary<string, string> mmrs, Dictionary<string, string> roles, Dictionary<string, string> heroes_attr)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (data.Version != null)
                    {
                        ...
                        {
                            if (heroes_translations.ContainsKey(player.Hero.ToLower()))
                            {
                                player.Hero = heroes_translations[player.Hero.ToLower()];
                            }
        ...
        public ParseStormReplay(long replayID, Uri replayURL, ReplaysNotProcessed notProcessedReplays, Dictionary<string, string> maps, Dictionary<string, string> maps_translations,
                                Dictionary<string, string> game_types, Dictionary<string, string> talents, Dictionary<string, string> seasons_game_versions, Dictionary<string, string> mmr_ids,
                                Dictionary<string, DateTime[]> seasons, Dictionary<string, string> heroes, Dictionary<string, string> heroes_translations, Dictionary<string, string> maps_short,
                                Dictionary<string, string> mmrs, Dictionary<string, string> roles, Dictionary<string, string> heroes_attr)
        {
        ...
        public void saveReplayData(LambdaJson.ReplayData data)
        {
            try
            {
                ...
                foreach (var player in data.Players)
                {
                    ...
                    {
                        if (!heroes.ContainsKey(player.Hero))
                        {
                        ...
                        {
                            player.Hero_id = heroes[player.Hero];
                        }
                    }
                    ...
                    {
                        if (!talents.ContainsKey(player.Hero + "|" + talent))
                        {
                            talents.Add(player.Hero + "|" + talent, insertIntoTalentTable(player.Hero, talent, conn));
                        }
                    }
                }
                ...
                for (var teams = 0; teams < data.TeamExperience.Length; teams++)
                {
                    ...
                    {
                        if (!(data.TeamExperience[teams][team_time_split].TeamLevel >= 10)) continue;
                        if (teams == 0)
                        {
                ...
                foreach (var player in data.Players)
                {
                    player.Score.FirstToTen = player.Team == 0 ? team_one_first_to_ten : team_two_first_to_ten;
                }
                    ...
                    */
                    ...
                    foreach (var ban in data.Bans)
                    {
                        for (var j = 0; j < ban.Length; j++)
                        {
                            ...
                            if (heroes_attr.ContainsKey(ban[j].ToString()))
                            {
                                ban[j] = heroes_attr[ban[j].ToString()];
                    ...
                    for (var i = 0; i < data.Players.Length; i++)
                    {
                        ...
                        {
                            if (!heroes_attr.ContainsKey(item.HeroAttributeId)) continue;
                            if (heroes_attr[item.HeroAttributeId] != data.Players[i].Hero) continue;
                        ...
                        {
                            if (j == i) continue;
                            if (data.Players[i].Hero != data.Players[j].Hero) continue;
                    ...
                    foreach (var player in data.Players)
                    {
                        ...
                        using (var cmd = conn.CreateCommand())
                        {
                            if (player.Talents != null)
                            {
                                switch (player.Talents.Length)
                                {
                                    case 0:
                                        cmd.CommandText =
                                                "INSERT INTO talents (replayID, battletag, level_one, level_four, level_seven, level_ten, level_thirteen, level_sixteen, level_twenty) VALUES(" +
                                                replayID + "," +
                                                player.battletag_table_id + "," +
                                                "NULL" + "," +
                                                "NULL" + "," +
                                    ...
                                    case 1:
                                        cmd.CommandText = "INSERT INTO talents (replayID, battletag, level_one) VALUES(" +
                                                          replayID + "," +
                                                          player.battletag_table_id + "," +
                                                          talents[player.Hero + "|" + player.Talents[0]] + ")";
                                        break;
                                    case 2:
                                        cmd.CommandText = "INSERT INTO talents (replayID, battletag, level_one, level_four) VALUES(" +
                                                          ...
                                                          player.battletag_table_id + "," +
                                                          talents[player.Hero + "|" + player.Talents[0]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[1]] + ")";
                                        break;
                                    case 3:
                                        cmd.CommandText = "INSERT INTO talents (replayID, battletag, level_one, level_four, level_seven) VALUES(" +
                                                          ...
                                                          talents[player.Hero + "|" + player.Talents[0]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[1]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[2]] + ")";
                                        break;
                                    case 4:
                                        cmd.CommandText = "INSERT INTO talents (replayID, battletag, level_one, level_four, level_seven, level_ten) VALUES(" +
                                                          ...
                                                          talents[player.Hero + "|" + player.Talents[1]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[2]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[3]] + ")";
                                        break;
                                    case 5:
                                        cmd.CommandText = "INSERT INTO talents (replayID, battletag, level_one, level_four, level_seven, level_ten, level_thirteen) VALUES(" +
                                                          ...
                                                          talents[player.Hero + "|" + player.Talents[2]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[3]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[4]] + ")";
                                        break;
                                    case 6:
                                        cmd.CommandText = "INSERT INTO talents (replayID, battletag, level_one, level_four, level_seven, level_ten, level_thirteen, level_sixteen) VALUES(" +
                                                          ...
                                                          talents[player.Hero + "|" + player.Talents[3]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[4]] + "," +
                                                          talents[player.Hero + "|" + player.Talents[5]] + ")";
                                        break;
                                    case 7:
                                        cmd.CommandText =
                                                "INSERT INTO talents (replayID, battletag, level_one, level_four, level_seven, level_ten, level_thirteen, level_sixteen, level_twenty) VALUES(" +
                                                ...
                                                talents[player.Hero + "|" + player.Talents[4]] + "," +
                                                talents[player.Hero + "|" + player.Talents[5]] + "," +
                                                talents[player.Hero + "|" + player.Talents[6]] + ")";
                                        break;
                                }
                    ...
                    {
                        if (Convert.ToInt32(seasons_game_versions[data.VersionSplit]) < 13) return;
                ...
                else
                {
                    if (badHeroName)
                    {
                        ...
                        insertIntoReplaysNotProcessed(replayID.ToString(), "NULL", data.Region.ToString(), data.Mode, data.Length.UtcDateTime.TimeOfDay.TotalSeconds.ToString(),
                                data.Date.ToString("yyyy-MM-dd HH:mm:ss"), data.Map.ToString(), data.VersionSplit.ToString(), "0", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), replayURL.ToString(),
                                "NULL", "Bad Hero Name");
                        ...
                        using (var cmd = conn.CreateCommand())
                        {
                            if (player.Talents != null)
                            {
                                switch (player.Talents.Length)
                                {
                                    case 0:
                                        cmd.CommandText =
                                                "INSERT INTO heroesprofile_brawl.talents (replayID, battletag, level_one, level_four, level_seven, level_ten, level_thirteen, level_sixteen, level_twenty) VALUES(" +
                                                replayID + "," +
                                                player.battletag_table_id + "," +
                                                "NULL" + "," +
                                                "NULL" + "," +
                                    ...
                                    case 6:
                                        cmd.CommandText =
                                                "INSERT INTO heroesprofile_brawl.talents (replayID, battletag, level_one, level_four, level_seven, level_ten, level_thirteen, level_sixteen) VALUES(" +
                                                replayID + "," +
                                                player.battletag_table_id + "," +
                                                talents[player.Hero + "|" + player.Talents[0]] + "," +
                                                talents[player.Hero + "|" + player.Talents[1]] + "," +
        ...
        private void updateGlobalTalentData(LambdaJson.ReplayData data, MySqlConnection conn)
        {
            foreach (var player in data.Players)
            {
                ...
                else if (player.Talents[1] == null || player.Talents[1] == "")
                {
                    cmd.CommandText += getHeroCombID(
                                               player.Hero_id,
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[0]]),
                                               0,
                                               0,
                                               0,
                                               0,
                ...
                else if (player.Talents[2] == null || player.Talents[2] == "")
                {
                    cmd.CommandText += getHeroCombID(
                                               player.Hero_id,
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[0]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[1]]),
                                               0,
                                               0,
                                               0,
                                               0,
                ...
                else if (player.Talents[3] == null || player.Talents[3] == "")
                {
                    cmd.CommandText += getHeroCombID(
                                               ...
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[0]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[1]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[2]]),
                                               0,
                                               0,
                                               0,
                                               0) + ",";
                ...
                else if (player.Talents[4] == null || player.Talents[4] == "")
                {
                    cmd.CommandText += getHeroCombID(
                                               ...
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[1]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[2]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[3]]),
                                               0,
                                               0,
                                               0) + ",";
                ...
                else if (player.Talents[5] == null || player.Talents[5] == "")
                {
                    cmd.CommandText += getHeroCombID(
                                               ...
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[2]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[3]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[4]]),
                                               0,
                                               0) + ",";
                }
                else if (player.Talents[6] == null || player.Talents[6] == "")
                {
                    cmd.CommandText += getHeroCombID(
                                               ...
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[3]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[4]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[5]]),
                                               0) + ",";
                }
                else
                {
                    cmd.CommandText += getHeroCombID(
                                               ...
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[4]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[5]]),
                                               Convert.ToInt32(talents[player.Hero + "|" + player.Talents[6]])) + ",";
                }
        ...
        private void updateGlobalTalentDataDetails(LambdaJson.ReplayData data, MySqlConnection conn)
        {
            ...
            foreach (var player in data.Players)
            {
                ...
                for (var t = 0; t < 7; t++)
                {
                    ...
                    {
                        case 0:
                            cmd.CommandText += Convert.ToInt32(talents[player.Hero + "|" + player.Talents[0]]) + ",";
                            break;
                        case 1:
                            cmd.CommandText += Convert.ToInt32(talents[player.Hero + "|" + player.Talents[1]]) + ",";
                            break;
                        case 2:
                            cmd.CommandText += Convert.ToInt32(talents[player.Hero + "|" + player.Talents[2]]) + ",";
                            break;
                        case 3:
                            cmd.CommandText += Convert.ToInt32(talents[player.Hero + "|" + player.Talents[3]]) + ",";
                            break;
                        case 4:
                            cmd.CommandText += Convert.ToInt32(talents[player.Hero + "|" + player.Talents[4]]) + ",";
                            break;
                        case 5:
                            cmd.CommandText += Convert.ToInt32(talents[player.Hero + "|" + player.Talents[5]]) + ",";
                            break;
                        case 6:
                            cmd.CommandText += Convert.ToInt32(talents[player.Hero + "|" + player.Talents[6]]) + ",";
                            break;
                    }
        ...
        private string insertIntoTalentTable(string hero, string talent_name, MySqlConnection conn)
        {
            if (hero == "")
            {
                ...
                hero = heroes_alt.ContainsKey(split[0]) ? heroes_alt[split[0]] : split[0];
            }
        ...
        private string saveToSeasonGameVersion(DateTime game_date, string game_version, MySqlConnection conn)
        {
            ...
            foreach (var s in seasons.Keys.Where(s => game_date >= seasons[s][0] && game_date < seasons[s][1]))
            {
                season = s;
            }
        ...
        private void insertIntoReplaysNotProcessed(string replayID, string parsedID, string region, string game_type, string game_length, string game_date, string game_map, string game_version,
                                                   string size, string parsed_date, string url, string processed, string errorMessage)
        {
            using var conn = new MySqlConnection(db_connect_string);
            conn.Open();
